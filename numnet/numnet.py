...
from numnet.numgnn import NumGNN
from numnet.util import convert_number_to_str
...
class NumNet(Model):
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        self._passage_weights_predictor_0_for_100 = torch.nn.Linear(modeling_out_dim, 1)
        self._passage_weights_predictor_3_for_100 = torch.nn.Linear(modeling_out_dim, 1)
    ...
    def forward(self,  # type: ignore
                ...
                passage_number_rank: torch.LongTensor,
                question_number_rank: torch.LongTensor,
        ...
        modeling_layer_input = self._modeling_proj_layer(merged_passage_attention_vectors)
        first_modeled_passage = self._dropout(self._modeling_layer_0(modeling_layer_input, passage_mask))

        question_modeling_layer_input = self._modeling_proj_layer(merged_question_attention_vectors)
        first_modeled_question = self._dropout(self._modeling_layer_0(question_modeling_layer_input, question_mask))
        ...
        encoded_passage_for_numbers = first_modeled_passage
        ...
        encoded_question_for_numbers = first_modeled_question
        ...
        number_rank = torch.cat((passage_number_rank, question_number_rank), -1)
        graph_mask = number_rank.unsqueeze(1).expand(batch_size, number_rank.size(-1), -1) \
                > number_rank.unsqueeze(-1).expand(batch_size, -1, number_rank.size(-1))
        graph_mask = graph_mask.long()

        all_number_mask = torch.cat((number_mask, question_number_mask), dim=-1)
        graph_mask = all_number_mask.unsqueeze(1) * all_number_mask.unsqueeze(-1) * graph_mask

        # neumerical reasoning
        d_node, q_node = self._gnn(
        ...
        gnn_info_vec = torch.zeros((batch_size, first_modeled_passage.size(1) + 1, first_modeled_passage.size(-1)), dtype=torch.float, device=d_node.device)

        clamped_number_indices = util.replace_masked_values(real_number_indices, number_mask, gnn_info_vec.size(1) - 1)
        gnn_info_vec.scatter_(1, clamped_number_indices.unsqueeze(-1).expand(-1, -1 , d_node.size(-1)), d_node)
        gnn_info_vec = gnn_info_vec[:, :-1, :]

        modeled_passage_list = [self._proj_fc(torch.cat((first_modeled_passage, gnn_info_vec), dim=-1))]
        ...
        if "addition_subtraction" in self.answering_abilities:
            passage_weights_0_100 = self._passage_weights_predictor_0_for_100(modeled_passage_list[0]).squeeze(-1)
            passage_weights_0_100 = masked_softmax(passage_weights_0_100, passage_mask)
            passage_vector_0_100 = util.weighted_sum(modeled_passage_list[0], passage_weights_0_100)

            passage_weights_3_100 = self._passage_weights_predictor_3_for_100(modeled_passage_list[3]).squeeze(-1)
            passage_weights_3_100 = masked_softmax(passage_weights_3_100, passage_mask)
            passage_vector_3_100 = util.weighted_sum(modeled_passage_list[3], passage_weights_3_100)

            num100_vec = torch.cat((passage_vector_0_100, passage_vector_3_100), dim=-1).unsqueeze(1)
            ...
            encoded_passage_for_numbers = torch.cat((num100_vec, encoded_passage_for_numbers), 1)
        ...
        if metadata is not None:
            ...
            for i in range(batch_size):
                ...
                if predicted_ability_str == "passage_span_extraction":
                    ...
                    answer_json["count"] = predicted_answer
                else:
